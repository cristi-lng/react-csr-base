...
import type { ItemFormData } from '~shoppingLists/components/shoppingListPage/listFormTypes';
...
function useAddItemForm({ addItem }: { addItem: (item: ItemFormData) => void }) {
  ...
  const formCtrl = useForm<ItemFormData>({ mode: 'onChange', defaultValues: buildInitFormData() });

  function buildInitFormData(): ItemFormData {
    return { name: null, quantity: null, price: null, purchased: false };
  }

  function handleAddItem(item: ItemFormData) {
    addItem(item);
    formCtrl.reset(buildInitFormData());
  }
